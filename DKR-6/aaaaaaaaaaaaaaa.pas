...
var
  ...
  begin
    ...
    while (currentNode <> nil) and (currentNode^.data <> existingValue) do
      currentNode := currentNode^.next; // переход к след. узлу
    if currentNode <> nil then // если узел существует
    begin 
      newNode := createNode(newValue); // создание узла с заданным знач.
      newNode^.prev := currentNode^.prev; // устанавка ссылки
  ...
  begin
    ...
    if currentNode <> nil then // если узел существует
    begin
      newNode := createNode(newValue); // создание узел с заданным значением
      ...
      if currentNode^.next <> nil then
        ...
        tail := newNode;
      currentNode^.next := newNode; // обновление след. узел текущего узла
    end;
  end;
  ...
  begin
    ...
    while (currentNode <> nil) and (currentNode^.data <> value) do
      currentNode := currentNode^.next; // ппереход к след. узлу
    if currentNode <> nil then
    begin
      if currentNode^.prev <> nil then
        currentNode^.prev^.next := currentNode^.next // обновление ссылку
      else
      ...
      Dispose(currentNode); // освобождение память 
    end;
  end;
