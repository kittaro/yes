...
begin
  ...
  if (fEdit.eName.Text= '') or (fEdit.eAuthor.Text= '') then exit;
  ...
  SG.Cells[1, SG.RowCount-1]:= fEdit.eAuthor.Text;
  SG.Cells[2, SG.RowCount-1]:= fEdit.CBNote.Text;
  SG.Cells[3, SG.RowCount-1]:= fEdit.eYear.Text;
...
begin
  ...
  fEdit.eAuthor.Text:= SG.Cells[1, SG.Row];
  fEdit.CBNote.Text:= SG.Cells[2, SG.Row];
  fEdit.eYear.Text := SG.Cells[3, SG.Row];
  ...
  if fEdit.ModalResult = mrOk then begin
    ...
    SG.Cells[1, SG.Row]:= fEdit.eAuthor.Text;
    SG.Cells[2, SG.Row]:= fEdit.CBNote.Text;
    SG.Cells[3, SG.Row]:= fEdit.eYear.Text;
...
begin
  ...
  SG.Cells[1, 0]:= 'Автор';
  SG.Cells[2, 0]:= 'Насколько качает';
  SG.Cells[3, 0]:= 'Год релиза';
  SG.Cells[4, 0]:= 'Есть на диске';
  SG.Cells[5, 0]:= 'Тип файла';
  SG.ColWidths[0]:= 200;
  SG.ColWidths[1]:= 200;
  SG.ColWidths[2]:= 200;
  SG.ColWidths[3]:= 150;
  ...
  if not FileExists(adres + 'Muzik.dat') then exit;
  ...
  try
    ...
    while not Eof(f) do begin
      ...
      //добавляем в сетку новую строку, и заполняем её:
        ...
        SG.Cells[4, SG.RowCount-1]:= BoolToStr(MyCont.Watched, True);
